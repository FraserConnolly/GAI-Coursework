/*
  Taken from https://gist.github.com/thelastpointer/c52c4b1f147dc47961798e39e3a7ea10#file-editorgraph-cs
*/
	...
	public class EditorGraph
	{
		...
		public EditorGraph(float _minX, float _minY, float _maxX, float _maxY, string _title = "", int _resolution = 48)
		{
			...
			Colors = new GraphColors
			{
				...
				WarningArea = new Color(0.75f, 0.1f, 0.1f, 0.25f)
			};
		}
		...
		public void Draw(float width, float height)
		{
            ...
            {
				DrawRect(area.Position, minY, area.Position + area.Width, maxY, area.Color, Colors.Outline);
            }
        ...
        public void AddWarningArea(float startValue, float width)
		{
			AddWarningArea(startValue, width, Colors.WarningArea);
		}


        public void AddWarningArea( float startValue, float width, Color color)
		{
            foreach (var pair in warningAreas)
            {
                if (pair.Position == startValue && pair.Width == width)
                    return;
            }

            warningAreas.Add(new HighlightAreaColorPair { Position = startValue, Width = width, Color = color });
        }
		...
		List<HighlightAreaColorPair> warningAreas = new List<HighlightAreaColorPair>();
