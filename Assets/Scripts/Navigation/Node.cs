...
namespace GCU.FraserConnolly
{
    public class Node
    {
        ...
        public static IReadOnlyList<Node> GetAllNodes(Map map = null)
        {
            ...
            if ( map == null && Node.s_map != null )
            {
                // check if a new map is being used.
                if ( s_map != GameData.Instance.Map )
                {
                    ClearNodes();
                    s_map = GameData.Instance.Map;
                }
            }
        ...
        {
            return GetNodeAtPoint(point.x, point.y, nodes);
        }
        ...
        {
            var e = BresenhamLine.GetBresenhamLinePoints( new Point( start.Coordinate.x, start.Coordinate.y), new Point( end.Coordinate.x, end.Coordinate.y));
        ...
        public static bool HasLineOfSightBetweenPoints( Vector3 start, Vector3 end )
        {
            var nodes = GetAllNodes();

            return HasLineOfSightBetweenNodes(
                GetNodeAtPoint((int)start.x, (int)start.y, nodes),
                GetNodeAtPoint((int)end.x, (int)end.y, nodes)
                );
        }
